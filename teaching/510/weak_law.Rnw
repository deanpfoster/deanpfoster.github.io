...
\begin{document}
\title{Weak law of large numbers}
...
\begin{itemize}
\item In class exam next Monday
\item Last section of Helms to read: 4.4 
\end{itemize}


\section*{Definition: Variance}
\begin{itemize}
\item $Var(X) = E(X - \mu)^2$ where $\mu = E(X)$
\item Mathemtically better than SD or $E(|X-\mu|)$
\item Draw some pictures
\end{itemize}
...
\item If $X \perp Y$, then $Cov(X,Y) = 0$.  Not the other way around!
(Need all polynomials, not just easy one.)
\item $Var(X+Y) = Var(X) + Var(Y) + 2 * Cov(X,Y)$
\item $Var(X+Y) = Cov(X+Y,X+Y) = Cov(X,X) + Cov(X,Y) + Cov(X,Y) + Cov(Y,X)$
\item $Var(\sum X_i) = \sum_{i,j} Cov(X_i,X_j)$
\end{itemize}
...
\item IID for short
\item $\{X_i\}_{i=1}^n$
\item Interested in $\sum X_i$
\item interseted in average $\overline{X}$
\end{itemize}

\section*{Computing means and variances of sums}

Consider sums of IID random variables:
\begin{itemize}
\item Compute $E(\sum X)$ (linearity)
\item Compute $E(\sum X)^2$ (algebra)
\item Compute $Var(\sum X)^2$ (easy by covariance formula)
\end{itemize}

\section*{Summaries}

Let $X_i$ be a sequence of IID random variables.  Let $S =
\sum_{i=1}^n X_i$.  Then:
\begin{eqnarray*}
E(S) & = & n E(X) \\
Var(S) & = & n Var(X) \\
E(S^s) = \phi_S(s) & = & \phi_X(s)^n
\end{eqnarray*}
If the RHS's exist.
