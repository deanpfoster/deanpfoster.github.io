...
\begin{document}
\section*{510/430H Extra question}
...
this distance.  Clearly if $t=0$, then $p_1(i) = 0$ for all $i$ not
equal to zero, and the distance between $p_1(i)$ and $q(i)$ will be
very close to $1$.  (In fact, it will be $1 - 1/n$.)  But theory says
...
the L1 distance between the simulated distribution and the true
distribution?
\end{enumerate}
(Note: there are several ways of answering the above problems.  Some
 will give approximations, some will give bounds, etc.  There is no
 definitive correct answer.  But please state what it is you are
 claiming--both in words, and if possible using mathematics.)
