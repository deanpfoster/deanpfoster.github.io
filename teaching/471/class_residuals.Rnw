...
\section*{Last time}
\begin{itemize}
\item Fit plus residuals
\item last time: fit
\begin{itemize}
\item doglegs (single)
\item doglegs (smooth)
\item doglegs (multiple)
\item spline (multiple and smooth)
\item connect the dots (too many multiple)
\end{itemize}
\item Today--the other half:residuals
\end{itemize}
...
$(1/21)^2$, and the second at $(1/22)^2$, etc.  This means that if you
did 100 tests, the entire chance of rejecting something by chance
alone would be:
\begin{displaymath}
1/21^2 + 1/22^2 + 1/23^2 \cdots + 1/120^2 < .05
\end{displaymath}
What is cool about this sequence if that even if you end up doing a
million tests, 
\begin{displaymath}
1/21^2 + 1/22^2 + 1/23^2 + 1/24^2 \cdots + 1/1000020^2 < .05
\end{displaymath}
it is still less than .05.  

Let's confirm this with in R:
  
<<Check of sum>>=

p.values <- 1/(20 + 1:1000000)^2
sum(p.values)

@ 
  
In fact, if you want to save a small amount, you can use:
\begin{displaymath}
1/19.5^2 +1/20.5^2 + 1/21.5^2 + 1/22.5^2 + 1/23.5^2 \cdots + 1/1000019.5^2 < .05
\end{displaymath}
<<Check of sum>>=

p.values <- 1/(19.5 + 1:1000000)^2
sum(p.values)
