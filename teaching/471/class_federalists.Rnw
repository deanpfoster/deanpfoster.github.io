...
We can also see this by color coding the dots:

<<fig=TRUE,echo=FALSE>>=

  colors <- c("red","white","black","gray","blue")[federal$author]

  plot(federal$num.char ~ federal$num.words, col=colors)
...
<<fig=TRUE,echo=FALSE>>=
  ...
  plot(federal$num.char/federal$num.words ~ federal$num.words, col=colors)
...
\begin{center}
<<results=tex,echo=FALSE>>=
regr <- lm(training01 ~ I(num.char / num.words) + 
...
\begin{center}
<<results=tex,echo=FALSE>>=
regr <- lm(training01 ~ I(num.char / num.words) + freq.the  + freq.of + freq.to + freq.and + freq.in + freq.a + freq.be + freq.that + freq.is + freq.which + freq.it + freq.by,
                        data=federal)
print(xtable(summary(regr)),floating=F,size="small")
@ 
\end{center}
