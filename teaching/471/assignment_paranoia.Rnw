...
(\href{assignment_paranoia.pdf}{pdf version}) 

\subsection{Three friends}

In class, we did a proof that if you had two friends, you could get as
 good a return as the better of these two friends by simply investing
 50 cents with each of them and letting it ride.  Well, suppose you
 are an extrovert and actually have three friends.  Creatively call
 them $A$, $B$ and $C$.  Prove that if you give each of them 1/3 of a
 dollar and let it ride, that your average return will be
 approximately as high as the best of these three.
...
\item You hire an financal accountant (they are cheap now that
 ...
 He defines $A_t = 1 + a_t$ and has everone starts out with one dollar.
...
$a_t$ between zero and one.  What is the farthest these two are from
...
\epsilon + \epsilon^2/2$.  For an exact bound, you
...
We can now put this all together: Your financial accountant then
 ...
 scheme, but does it have horse race properties.  Define $C_t = 1 +
