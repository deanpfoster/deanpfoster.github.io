...
\item Rewrote evolution homework last night.  Should be much easier
to follow now.
\end{itemize}
...
\section*{Where it all started: IQ}

\begin{itemize}
\item Spearman (research done during 1904-1920)
\begin{itemize}
\item rank correlation
\item psychology, ``G'' in IQ
\item came up with factor analysis: basicly the same idea as PCA.  We
will use it for discussion of ideas.
\end{itemize}
\end{itemize}

\subsection*{IQ: background}


\begin{itemize}
\item Spearman found grades in various classes correlated
\item conjectured there was one thing that did it all
\item called G
\end{itemize}

\subsection*{What are the class weights?}

\begin{itemize}
\item classes: Math, English, history, PhEd
\item weights: .3 M + .4 E + .2 H + 0 PhEd
\item tells how much each class is realted to IQ
\item FACT: reaction time is related, motor speed isn't.  Takes care
to distinguish these two in tests.
\end{itemize}

\subsection*{What are the individual scores?}

\begin{itemize}
\item Each student has g = .3 M + .4 E + .2 H + 0 PhEd
\end{itemize}

\subsection*{Putting them together}

Instead of storing all grades, we can store two things:
\begin{itemize}
\item student scores: g
\item class loading: (.3,.4,.2,0)
\item Compute estimated grade in math by: g * .3
\item Compute estimated grade in PhEd by: g * 0
\item So we get nothing out of PhEd
\end{itemize}
