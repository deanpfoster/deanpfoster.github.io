...
\item This problem will have you compute some PCA's for the market.
  We will first have to download the data.
...
\item Ask R to compute the principle components (prcomp).    Is
this something you could buy?  (I.e. is there a constant term in it?)
Play around with the ``center'' and ``scale'' function to end up with
things you can buy.
...
they look different than the deciles?
\end{enumerate}
