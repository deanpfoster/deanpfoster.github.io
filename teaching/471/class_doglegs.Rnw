...
\maketitle
(\href{http://gosset.wharton.upenn.edu/~foster/teaching/471/class_doglegs.pdf}{online version})

\section*{Story time: Publishing books}

\begin{itemize}
\item Information wants to be free
\begin{itemize}
\item I could tell you who said that--but wiki is down today
\end{itemize}
\item Accedemics write papers for free
\item Most musicians (as in numbers) write music for free
\item Book authors no longer need book publishers
\item So, copyright is kinda obselete
\item I'm fairly radical on this and push against comercial publishers in accemia
\item Mention my two chairs
\item Melone (is chair from upper east side) Melony is sponsor of bill (from upper east side)
\end{itemize}
...
discussed it.  It is stuff you should know how to do already.  The
main issue is using R to do it.
\end{itemize}
...
Nasty stuff:
\begin{itemize}
\item Jumps (notice it is still ``smooth'' almost everywhere)
\item piecewise linear function (again with the almost everywhere)
\item exerce for math types: Show $f$ is piecewise linear iff $f'$ is
piecewise constant.
\end{itemize}
