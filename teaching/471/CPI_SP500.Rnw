...
m <- function(x,n)
 {
   c(rep(0,n),x[(n+1):(length(x))]);
 }
                                                

clean <- data.frame(date=both$DATE,  market = both$vwretd   , cpi = both$cpiret, riskfree = both$t30ret)

clean$cpi.smooth <- (clean$cpi + m(clean$cpi,1) + m(clean$cpi,2) + m(clean$cpi,3) + m(clean$cpi,4) + m(clean$cpi,5))/6
