...
\begin{itemize}
\item Do on homeworks 2 and 3.
\end{itemize}

\section{The telephone}
\begin{itemize}
\item Thoughts are QM which is analog
\item Words are digital
\item Sound is analogue
\item encoded to digital packets by phone
\item carried by wire as analogue
\end{itemize}

\section{Basic physics: limists of encoding}
...
\section{What does the human equiptment look like?}
