...
\begin{eqnarray*}
\hat{P}_2(z|x,y) &=& P^*(z|x,y) \hbox{if $C(x,y,z) > 0$} \\
\hat{P}_2(z|x,y) &=& \alpha_2(x,y) \hat{P}_1(z|y) \hbox{otherwise} \\
\hat{P}_1(z|y) &=& P^*(z|y) \hbox{if $C(y,z) > 0$} \\
\hat{P}_1(z|y) &=& \alpha_1(y) \hat{P}_0(z) \hbox{otherwise} \\
\hat{P}_0(z) &=&  P^*(z)  
\end{eqnarray*}
\end{itemize}
