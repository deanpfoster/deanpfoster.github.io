...
\begin{itemize}
\item HW 2 due Thursday and HW 3 next tuesday?
\item HW 1 collected today
\item HW 4 is posted.  Due in 2 weeks.
\end{itemize}
...
Top down vs bottom up.  Do example.


\section*{Trick from Fubinichi}
...
\begin{itemize}
\item Easy to add probabilities to divisions
\item Called ``statistical parsing''
\item Well discuss it Collins' version of it next time
\item So please read 13 and 14 to prepare for next class
\end{itemize}
