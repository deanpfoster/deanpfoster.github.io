...
\section*{Easy case: observed = real}
\begin{itemize}
\item Ignore $U$ (i.e. make it an dentity)
\item Equations reduce to:
\begin{eqnarray*}
\vec{b}_1 & = & P_1 =\pi_0 \\
P^T_{2,1} \vec{b}_\infty & = & P_1 \\
P_{2,1} B_x & = & P_{3,x,1}
\end{eqnarray*}
\item Our $A_x$ is then simply $T_{x.}$.  The transition probabilites
