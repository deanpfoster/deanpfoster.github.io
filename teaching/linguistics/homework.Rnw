...
\begin{document}
\section*{Homework 1: Regular expressions}

\begin{enumerate}
\item Read chapters 1 and 2 from JM.  
\item From the book JM: 2.1, 2.4, 2.8

\item Statistical analysis of two regular expressions.  Consider the
following two regular expressions do.  
\begin{enumerate}
\item First Regex:
\begin{verbatim}
(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])
\end{verbatim}
(Note, if you have trouble getting that to run, you can try the
following which is a much watered down version which is in R format:
<<>>=
...
\end{verbatim}
\end{enumerate}
Now you could read them and understand them.  But that would be
...
aren't legal strings.  Once you a theory, come up with some directed
tests.  So for example, if you think it is a parse for LAST NAME,
comma, FIRST NAME, then give it some examples that cause it to match
like, ``Washington, George'' and ``Mannard Smith, John'' both of which
are legal and something like, ``H. P. Lovecraft'' which wouldn't be
legal since it is in the wrong order.  

Now that you have an idea of what these are searching for, grab a
large piece of text (say 1 million words or so) and count how many
matches there are to both of these regular expressions.



\end{enumerate}

\section{Homework 2: Words}

\begin{enumerate}
\item Read chapter 3 and 4? of JM
\item JM: 3.5 and 3.6, 3.10.
\item JM: 4.2-4.7
\end{enumerate}

\section{Homework 2: authorship}
